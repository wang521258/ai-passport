...
//   确定 短按 = 昼夜切换（1 秒过渡）
//
// 第一版**故意没做**：首页整屏设计稿 / 开局三屏（选条数图章、分色）/
//   荷叶与浮萍 / 成长与存档 / 夜间月相。别以为漏了，是排在后面。
//
...
static void bands_mark_dirty(void)
{
    for (int b = 0; b < 5; b++) {
        ...
        dirty_add_ext(0, (int)floorf(bd->y - bd->amp - bd->lw),
                      KW - 1, (int)ceilf(bd->y + bd->amp + bd->lw));
    }
}
...
static void dirty_add_ext(int x0, int y0, int x1, int y1)
{
    ...
    for (int i = 0; i < s_nrect; i++) {
        if (x0 > s_rc[i].x1 + 1 || x1 < s_rc[i].x0 - 1 ||
            y0 > s_rc[i].y1 + 1 || y1 < s_rc[i].y0 - 1) continue;   // 不相交
        if (x0 < s_rc[i].x0) s_rc[i].x0 = x0;
        if (y0 < s_rc[i].y0) s_rc[i].y0 = y0;
        if (x1 > s_rc[i].x1) s_rc[i].x1 = x1;
        if (y1 > s_rc[i].y1) s_rc[i].y1 = y1;
        for (int again = 0; again < 4; again++) {                  // 合并后可能又与别的相交
            int merged = 0;
            for (int k = 0; k < s_nrect; k++) {
                if (k == i) continue;
                if (s_rc[i].x0 > s_rc[k].x1 + 1 || s_rc[i].x1 < s_rc[k].x0 - 1 ||
                    s_rc[i].y0 > s_rc[k].y1 + 1 || s_rc[i].y1 < s_rc[k].y0 - 1) continue;
...
static void do_feed(void)
{
    ...
    for (int k = 0; k < 8; k++) {                   // 只出涟漪、不产食物的"雨点"
        ...
        pel_t *pe = &s_pel[s_npel++];
        pe->sx = 0; pe->sy = 0;
        pe->tx = rnd_f(10, KW - 10); pe->ty = rnd_f(16, KH - 22);
