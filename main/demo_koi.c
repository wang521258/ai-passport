...
static inline float fsin_t(float a)
{
    ...
    PCNT(trig);                          /* ★ 三角函数查表调用（内含一次 floorf 软浮点） */
    int   i0 = (int)floorf(t);
    PCNT(floor);
    float fr = t - (float)i0;
    i0 &= 1023;
...
static inline void bb_pts(const float *xy, int n)
{
    if (!s_bb_on) return;
    for (int i = 0; i < n; i++) {
        int x = (int)floorf(xy[i * 2]), y = (int)floorf(xy[i * 2 + 1]);
    ...
    if (hw >= 1.2f) {
        ...
        for (int i = 0; i < n; i++) {
            for (int k = 0; k < 10; k++) {
                ...
                disc[k].x = (int32_t)rne_f2i((xy[i * 2] + fcos_t(t) * hw) * 256.0f);
                disc[k].y = (int32_t)rne_f2i((xy[i * 2 + 1] + fsin_t(t) * hw) * 256.0f);
            }
...
static int build_palette(float night)
{
    ...
    for (int i = 0; i < PI_NPAL; i++)
        for (int c = 0; c < 3; c++)
            s_pal[i][c] = (uint8_t)(((int)DAY_PAL[i][c] * k) >> 8);
    water_row_build();
    return 1;
}
...
static KOI_HOT void water_rect(int x0, int y0, int x1, int y1)
{
    for (int y = y0; y <= y1; y++) {
        ...
        for (int x = x0; x <= x1; x++) o[x] = c;      /* 整行同色，无查表无混合 */
    }
...
/* ★ 荷叶画法的粗细档（28 轮）。王总原话：
   ...
   ────────────────────────────────────────────────────────────────── */
#define LILY_LV      1
#define LILY_GAP_IN  0.34f   /* 缺口内顶点半径 / 叶半径（0 = 切到圆心，1 = 切到叶缘） */
#define LILY_VEIN_N  9       /* 平面档叶脉条数（精细档另给 11） */
#define LILY_VEIN_A  105     /* 平面档叶脉 alpha ≈0.41 */
...
/* 局部圆弧点列（叶缘内侧反光用）—— 只有 LILY_LV=0 的精细版会调，
   平面版不画那道反光。整段包在 #if 里：既是"删干净"，也避免 -Wunused-function
   （static 函数没人调，-Werror 下直接编不过）。 */
#if LILY_LV == 0
...
static void lily_paint(const lily_t *L, float th)
{
...
#if LILY_LV == 0
    /* 只有精细版用得上 —— 否则 -Wunused-variable 会在 -Werror 下直接编不过。 */
         ...
         还补在原点就等于补到叶心去了（白补一个点，而且被叶芯盖住看不见）。 */
    {
        ...
        for (int k = 0; k < 8; k++) {
            ...
            d[k].x = (int32_t)rne_f2i((wx + fcos_t(t) * 0.5f) * 256.0f);
            d[k].y = (int32_t)rne_f2i((wy + fsin_t(t) * 0.5f) * 256.0f);
        }
    ...
    const float v0 = 0.30f, v1 = 0.92f;   /* 从 0.30r 起笔 → 中心留给叶芯，不成"蒲公英" */
...
static void pond_init(void)
{
         ...
         插错地方会让波带和波光点整体平移（网页版 initPond 也是这个次序）。 */
    {
        ...
        while (s_nlily < MAX_LILY && guard++ < 900) {
            ...
            L->bob  = rnd_f(0, 6.28f);
            /* ★ 29 轮：0.13~0.25 → 0.26~0.36。
               叶缘开口宽 = 2r·sin(gap)：r=17 时从 4.4~8.4px 提到 8.7~12.0px ——
               前者被 1px 描边从两侧各糊掉 0.5px 就基本没了（王总："没有三角开口"）。
               ⚠️ 只改**取值范围**，rnd 调用次数不变 → 全局随机序列一动不动。 */
            L->gap  = rnd_f(0.26f, 0.36f);
...
static void koi_step(koi_t *k, float dt)
{
    ...
    if (ti >= 0 && ti < s_nfood &&
        ...
        /* ★ 第 34 轮补标脏：这颗饲料的像素原来靠"鱼的大脏框顺手扫过"才被擦掉
           ...
           现在自己报一份，两件事解耦。 */
        dirty_add_ext((int)floorf(s_food_x[ti]) - 3, (int)floorf(s_food_y[ti]) - 3,
...
static void step(float dt)
{
    ...
    for (int i = s_npel - 1; i >= 0; i--) {               // 饲料下落 / 雨点落水
        ...
        if (pe->delay > 0) {
            ...
            dirty_add_ext((int)floorf(pe->x) - 3, (int)floorf(pe->y) - 3,
        ...
        if (pe->t >= 1.0f) {
            ...
            int qx0 = (int)floorf(px0), qy0 = (int)floorf(py0);
            ...
            int rx0 = (int)floorf(ex0), ry0 = (int)floorf(ey0);
    ...
    for (int i = s_nfood - 1; i >= 0; i--) {
        ...
        dirty_add_ext((int)floorf(s_food_x[i]) - 3, (int)floorf(s_food_y[i]) - 3,
...
{
    for (int i = 0; i < HM_PXN; i++) s_fb[i] = HM_PAL[(uint8_t)HM_IDX[i]];
}
...
/* 按键分派。逐条对齐网页版的 fire(name)：feed(↑) / tap(↓) / night(OK)。
   ★ OK 在开局屏是"确定"、在池内是"昼夜切换" —— 同一个物理键，两屏语义不同。
     长按 OK（返回菜单）仍由 main.c 统一拦截，与本函数无关。 */
