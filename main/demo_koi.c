...
enum {
    ...
    PI_KGOLD_DK,     /* 阴影   #E0A63F = 224,166,63 —— 腹侧 / 尾根压暗（59 轮提亮 +49，62 轮保留） */
    PI_KGOLD_LT,     /* 亮部   #FFD34E = 255,211,78 —— 头部补偿（62 轮退回 53 轮旧值） */
    PI_KGOLD_HI,     /* 高光   #FFF0A0 = 255,240,160 —— **鱼背**中央窄带（62 轮退回旧值） */
    PI_KSCALE,       /* 鳞片点 #FFF7D1 = 255,247,209 —— 背上几粒很小的点（62 轮退回旧值） */
    PI_KGOLD_TL,     /* 尾梢浅金 #FFE28C = 255,226,140 —— 尾鳍梢（62 轮退回旧值） */
...
static const uint8_t DAY_PAL[PI_NPAL][3] = {
    ...
    {244, 241, 232}, {229,  57,  45}, {246, 182,  65}, {252, 204, 176},
    ...
    {164, 226, 118},
    /* ★★ 第 53 轮：御黄金的金属金层次（王总指定色值）
       ★★★ 第 59 轮：王总看真机后说「金色的鱼颜色还是有点深，需要浅一点，
            色差不要太大」⇒ 那一版把 6 档**全部**提亮了。

       ★★★★ 第 62 轮（本轮）：王总看了第 59 轮那版真机，说
            「**你直接全部提亮了，差点意思**，之前挺好的鱼背。
              我希望的是**只在前面的暗色部位提亮、跟现在一样**，
              **后面保持之前的颜色**」
            ⇒ 他要的不是"整条鱼变浅"，而是**只提暗端、亮端原样退回**。

         这 5 档各自画在鱼的什么地方（见 koi_draw 第 2955 行起的四层 grad）：
           PI_KGOLD_DK → ① **两侧（腹）压暗** ④ **尾根压暗**   ★ 这就是"暗色部位"
           PI_KGOLD    → 鱼身主色（平涂底）
           PI_KGOLD_LT → ③ **头部**补偿（越靠头越亮）
           PI_KGOLD_TL → 尾鳍**梢部**浅金
           PI_KGOLD_HI → ② **背部中央**窄高光                ★ 这就是"鱼背"
           PI_KSCALE   → 鳞片点 + 背上 1~2 个小高光点

         档位          53轮旧值   59轮全提亮   **62轮（本轮）**   说明
         PI_KGOLD_DK   185,104,18  224,166,63  **224,166,63**   ★ 暗色档，保持 59 轮
         PI_KGOLD      245,166,35  248,198,94  **246,182, 65**  只为了夹在 DK 与 LT 之间
         PI_KGOLD_LT   255,211,78  255,222,147 **255,211, 78**  ← 退回旧值
         PI_KGOLD_TL   255,226,140 255,234,176 **255,226,140**  ← 退回旧值
         PI_KGOLD_HI   255,240,160 255,243,194 **255,240,160**  ← 退回旧值（**鱼背原样**）
         PI_KSCALE     255,247,209 255,249,220 **255,247,209**  ← 退回旧值

         亮度：151 / 164 / 181 / 207 / 218 / 237
         （59 轮全提亮是 151/180/208/222/231/241 —— 亮端那四档本轮全部退回）

       ■ 为什么 PI_KGOLD（主色）也动了一点（149 → 164）
         它必须夹在 DK(151) 与 LT(181) 之间。若退回旧值 149，就会 **149 < 151**，
         主色比阴影还暗 ⇒ "背暗腹亮"整个反过来，鱼身立体感错乱。
         所以只能小幅提到 164 —— 比旧值仅 +15，远小于 59 轮的 +31。

       ■ 结果
         · 鱼背高光（HI/KSCALE）**与提亮前一模一样** —— 王总"之前挺好"的那部分保住了
         · 头部补偿（LT）、尾鳍梢（TL）同 53 轮
         · 只有腹侧/尾根那个暗色档变浅了（102 → 151），这正是王总说"深"的地方
         · 明暗跨度 237−102=135 → 237−151=**86**（收窄 36%）
       ⚠️ 只给 pat==1（御黄金）用；红白那套（PI_KBODY/KSPOT）一个字节都不动。 */
    {224, 166,  63}, {255, 211,  78}, {255, 240, 160}, {255, 247, 209},
    {255, 226, 140},
...
/* ★★ 第 63 轮：动态水面光网 caustic（王总：「水面只是图形的形式的动态光影
   ...
   ★★★★ 王总真机看过后的裁定（第 63 轮收尾）：「不太好实现 这个不满意 还是退回上一版吧」
      ⇒ **默认改回 0（关）**，恢复到第 62 轮 v28 那种静态水面。
      代码整体保留（模板 koi_caustic.h + caustic_overlay + 台架入口都还在），
      因为：
        · 换公式再试只需要改 gen_caustic.py 重烘 + 把这里拨回 1，不用重写；
        · 台架 `KOI_CAUSTIC=1 KOI_CAUSTIC_GAIN=n` 仍能出对比帧，随时可以再评。
      ⚠️ 别把这段实现删掉 —— 删了下次想试要重写一遍（铁律 9 那一族：
         别为了"当前不用"就删掉已经验证过的可复用实现）。 */
#define LOOK_CAUSTIC_DEF 0
...
/* ★★ 第 46 轮：头部两个可调量（王总「鱼的最前面 尖尖的 需要稍微打磨下」）
   ...
   ★★★ 第 61 轮定档：王总看了 `_preview/koi_body_shape_分格对比.png` 的四格对比，
      说「**红色（= 现状整体）挺好**」，但又单独圈出**头顶那顶"尖帽"**：
      「这个地方稍微圆润宽点 不然鱼很小 看着很尖」。
      ⇒ 结论是**保留现状的身体轮廓，只磨头顶那一块**。所以：
        · KDEPTH 后 5 段（颈/腹/后腹/尾柄/尾根）**一字不动**；
        · KOI_KDEPTH0  0.50 → **0.55**  头段横向加宽（头顶不再窄）
        · KOI_HEADCAP  1.70 → **1.25**  吻端凸出收短（尖帽磨圆钝）
      帽高 = hw0 × cap：22.3 → 17.2（-23%），头顶横向 +10%。
      ⚠️ 为什么这回**同时**动了 KDEPTH0（第 46 轮当时特意不动它）：
          第 46 轮王总说的是"最前面尖" ⇒ 只该修鼻子；
          本轮他说的是"**圆润宽**点" ⇒ 光把帽压扁会更扁，**必须同时加宽**才不会越改越瘦。
      ⚠️ 两个都是**纯绘制量**，不进物理（鱼速/吃食/边界靠 kh = L·grow·0.55），
          所以同一帧号下鱼位/朝向逐位不变，可以直接 A/B。
      ⚠️ 但 KDEPTH0 加大会**加大头部 AABB** ⇒ 改完必须看台架「鱼脏盒最大欠缺」。 */
#ifndef KOI_KDEPTH0
#define KOI_KDEPTH0 0.55f
#endif
#ifndef KOI_HEADCAP
#define KOI_HEADCAP 1.25f
#endif
...
/* ★ 第 64 轮 v34：顶点数 11 → 18。
   11 个顶点在 4.5px 斑上每段才 1.4px，咬不出"分叶"。
   18 个顶点 = 边缘多 64% 的细节，是团块轮廓的最低要求。
   ⚠️ 上限没再卡：再多就是浪费（fcos_t 是查表，18 个顶点已足够细）。 */
#define KOI_SPOT_SEGS   18
#endif
#ifndef KOI_SPOT_JIT
/* ★ 第 64 轮：0.34 → 0.42。
...
/* ★ v39：1.32 → **1.38**（王总"红色占比要多点"）。
   ⚠️ 只加 sw 的空间有限（受 lim 约束），所以这里再整体放大 4.5% 线性（面积 +9%）。
      sl 也会跟着 +4.5% ⇒ 半长 0.68 段 vs 间距 1.20 段 ⇒ 仍有轻微相接但不连片。 */
#define KOI_SPOT_SCALE  1.38f
#endif
...
#ifndef GOLD_HEAD_A
/* ★ 第 55 轮：90 → 130。
   v19 的 90 补不住头部描边多出来的那一份暗（王总：「鱼头处颜色深」）；
   v20 试过 135 但把范围拉宽到 −0.75Hl，中段被盖 ⇒ 背脊高光糊了（「背部还不如 v19」）。
   ⇒ 本轮：**范围收回到只在头部**（零区扩到 +0.20Hl，中段 α=0），alpha 提到 130。
   这样"后面"（中段/尾部）**逐字节等于 v19**，只有头端被提亮。 */
#define GOLD_HEAD_A    130
#endif
#ifndef GOLD_TAIL_A
...
static void make_spots(koi_t *k)
{
    ...
    for (int i = 0; i < n; i++) {
        int big = (i % 2 == 0);
        /* ★ v38：**两种分布模式**（王总"鱼头鱼中鱼后 / 鱼中鱼后 概率"）。
           n=3 ⇒ **头+中+后**：u 从 0.90 起 ⇒ i=0 落在段0末/段1前 = **鱼头**
           n=2 ⇒ **中+后**    ：u 从 1.70 起 ⇒ 两块都在身体后半
           ⚠️ 仍是那 1 次 rnd_f 决定的 n，铁律 20 不动。

           实算（head ∈[0,0.6] ⇒ 抖动 ∈[-0.3,+0.3]）：
             n=3: 头 0.95~1.54 · 中 1.93~2.52 · 后 2.92~3.51   相邻距 ≈1.0 段
             n=2: 中 1.75~2.40 · 后 2.83~3.47                   相邻距 ≈1.1 段
           ⚠️ 头段（u<1）能放斑是因为下面加了 **sl 按端点 room 收紧**，
              否则 p_lo = tt − 半长 < 0 会把半边顶点 clamp 成一条（v36b 的教训）。 */
        /* ★ v38c：**非均匀映射** —— 用 i/(n−1) 把首块钉在起点、末块钉在终点，
           而不是 i/n 那种"首块就跑到 1/3 处"的均分。
             起点 ua：n=3 ⇒ 0.88（**鱼头**，段0）   n=2 ⇒ 1.70（鱼中，段1）
             终点 ub：3.50（**鱼后**，段3）
           实算（head∈[0,0.6] ⇒ 抖动 ±0.105）：
             n=3: 头 0.78~0.99(段0) · 中 2.06~2.27(段2) · 后 3.40~3.61(段3)
             n=2: 中 1.60~1.81(段1)  · 后 3.40~3.61(段3)
           ⇒ 相邻间距 ≈1.31 段（v38b 的 i/n 均分只有 0.98 段，斑更容易分开）。
           room 检查：头斑 u=0.88 ⇒ 可容半长 0.78 段 > 实际 0.77 段 ⇒ **不会被压小**。 */
        /* ★ v40：改成**查表式**把三块位置明确钉住。
           v39 用 i/(n−1) 线性映射时，n=3 的中间块落在 u=2.00（段1/段2 交界），
           实测几乎全被推到**段1**（段2 只统计到 12 个）—— 而段2 才是腹部最宽处
           （KDEPTH[2]=1.00）⇒ lim 最松 ⇒ sw 能放到最大。
           ⇒ 直接钉死：
             n=3 ⇒ 头 0.80(段0) / 中 2.25(段2) / 后 3.40(段3)
             n=2 ⇒     中 1.60(段1) / 后 3.40(段3)
           抖动 ±0.24 段（head 派生，比 v39 的 ±0.105 大，避免每条鱼长得一样）。
           ⚠️ 不新增 rnd_f 调用（铁律 20）。 */
        float u;
        if (n == 3) {
            u = (i == 0) ? 0.80f : ((i == 1) ? 2.25f : 3.40f);
        } else {
            u = (i == 0) ? 1.60f : 3.40f;
        }
        u += (head - 0.30f) * 0.80f;
        ...
        /* ★ v38：下限从 1 放开到 **0** —— 王总"鱼头最前面没有"。
           v36b 设成 1 是为了躲开端点截断，现在截断由下面
           **sl 按端点 room 等比收紧**负责 ⇒ 头段可以放斑了。 */
        if (sg < 0) sg = 0;
        ...
        /* ★ v37b：相邻斑**交替长在脊线两侧**（治"糊成一块"），幅度略收。
           v37 第一版用 ±(0.12~0.30)，平均 |so|≈0.21 让斑横向更靠边 ⇒
           鱼身 AABB 增大 ⇒ 脏区从 35% 涨到 60% ⇒ 9 鱼满载帧时间从 116ms 翻到 196ms。
           ⇒ 改成 ±(0.08~0.20)，平均 |so|≈0.14（≈ v36 水平），分块仍由
             "弧长均匀 + i 奇偶交替"保证（横向 sw_px≈2.5、半宽 7px 左右够分开）。 */
        float so = ((i % 2) == 0 ? 1.0f : -1.0f) * rnd_f(0.08f, 0.20f);
        ...
        /* ★ v39：margin 0.06 → **0.04** —— 给 sw 更多空间（提占比）。
           绘制处 koi_draw 的 lim_r 仍用 0.06 做**第二道保险**，
           所以这里放松不会导致溢出（v35/v36/v38 的 [SPOT] 判据一直在盯着）。 */
        float lim = (hw - 0.04f - fabsf(so)) / KOI_SPOT_SCALE;
        if (lim < 0.30f / KOI_SPOT_SCALE) lim = 0.30f / KOI_SPOT_SCALE;
        /* ★ 第 64 轮 v34：sw 区间再收一点（0.40~0.50 → 0.36~0.46）。
           原因：凸出 5% 留给溢出余量 + 与凸出联合控制让头部窄段的斑**只能凹
           不能凸**（lim_r 在那里约 0.55）。
           ⇒ sw 上限 0.46 × 1.15 × 1.05 = 0.555 < 腹部 lim 0.70-0.13 = 0.57
             —— 头部临界，所以配合绘制处每个顶点的 lim_r clamp 一起保安全。 */
        /* ★ v41：sw 区间整体抬 11%（王总"红色占比要多点"）。
           为什么加 sw 而不是 sl：**纵向连片只由 sl 决定**，sw 是横向的、
           不影响纵向间距 ⇒ 加 sw 提占比不会让斑连成一片，也不会变圆
           （长宽比 1.86 → 1.69，仍明显是沿鱼身的长条色块）。
           ⚠️ 只改 rnd_f 的区间，调用次数不变（铁律 20）。 */
        float sw = big ? rnd_f(0.40f, 0.50f) : rnd_f(0.24f, 0.33f);
        ...
        /* ★ v38a：**sl 按"离端点的距离 room"等比收紧** —— 这是"鱼头最前面
           也能放斑"的前提。
           p = sg + tt + rx/segl，rx 最远 ±1.10·sl；段0 且 tt 小时
           p_lo = tt − 半长 < 0 ⇒ **半边顶点全被 clamp 到 0** ⇒ 斑被削成长条。
           半长的段数 = sl · SCALE · KSEG（sl 相对 L，segl ≈ L/KSEG）；
           ⇒ 要求 sl·SCALE·KSEG + 0.10 ≤ room = min(tt, KSEG−(sg+tt))。
           超了就 **sl 与 sw 等比缩**（保持长宽比 ⇒ 不出现圆点，只是整体小）。
           实算 sg=0, tt=0.95 ⇒ room=0.95 ⇒ 半长上界 0.85 段 ≈4.9px
             ⇒ 全长 ≈9.8px —— **看得见的色块**，不是噪点 ✓ */
        {
            float room_lo = (float)sg + tt;          /* ★ 绝对弧长，不是段内 tt */
            float room_hi = (float)KSEG - ((float)sg + tt);
            float room    = (room_lo < room_hi) ? room_lo : room_hi;
            float sl_seg  = sl * KOI_SPOT_SCALE * (float)KSEG;
            float sl_seg_max = room - 0.10f;
            if (sl_seg_max < 0.10f) sl_seg_max = 0.10f;
            if (sl_seg > sl_seg_max) {
                float k3 = sl_seg_max / sl_seg;
                sl *= k3;
                sw *= k3;          /* ★ 等比：形状不变，只是整体小 */
            }
        }
        /* ★ v39：**删掉 v38b 的 `if (n==3) ×0.82`**。
           v38 用缩小 33% 面积换取"完全不接触"，代价太大（王总："红色占比要多点，
           还不如上一版"）。王总也说过"可以概率连一起" ⇒ 允许相邻斑边缘轻微相接。
           现在 n=3 间距 1.20 段、半长 ≈0.65 段 ⇒ 两块半长 1.30 > 1.20
           ⇒ 边缘轻微相接（真实锦鲤也常这样），但**不会糊成一整片**：
           靠的是"左右交替在脊线两侧"+ 不规则边缘 + 每斑色调深浅不同。 */
...
static void koi_draw(koi_t *k)
{
    ...
    if (isGold) {
        ...
        /* ③ 头部**补偿**：u 取**指向头**的方向（切向是头→尾，取负）。
           ★★ 第 55 轮重做（王总：「鱼头处颜色深」「改成后面的一样的颜色就好了」）。
           ★ 为什么头部会偏深（不是 ③ 不够亮那么简单）：
             鱼头是**尖**的（KDEPTH[0]≈0.7），而体缘暗线 stroke（PI_KEDGE #B96812 深褐
             α133）是沿整圈描的 —— 头越窄，两条描边离得越近，几乎**糊满整个头部**；
             身体中段宽，描边只占边缘一小条。所以同样的描边，在头上的"暗占比"高得多。
             ⇒ 头部要补的不是"更亮"，是"把描边多出来的那一份暗抵消掉"，
               让它读起来跟身体中段一个色。
           ★ 为什么**零区要扩到 +0.20Hl**（v19 是 −0.20Hl）：
             第 54 轮 v20 试过把范围拉宽到 −0.75Hl，结果中段也被 #FFD34E 盖住，
             把 ② 那条 #FFF0A0 背脊高光糊掉了（王总：「背部还不如 v19」）。
             ⇒ 现在反过来：**中段及以后一律 α=0**（完全等于 v19，背脊高光原样保住），
               只在 s > +0.20Hl（真正靠头那一段）才渐强到 GOLD_HEAD_A。
           ⚠️ s 的符号：u = −t ⇒ **s>0 = 头侧，s<0 = 尾侧**（Hl = 鱼身半长）。 */
        ...
        g.s2 = (int32_t)( Hl * 0.75f * 256.0f); g.a2 = GOLD_HEAD_A;
        ...
        /* ★★ 第 55 轮：御黄金**不再加**头顶小高光。
           王总第 55 轮原话「颜色改成后面的一样的颜色就好了」——
           他要的是"头和身体一个色"，不是"头上多一个亮点"。
           （我第 55 轮第一版加了个 1.25px 的 PI_KGOLD_HI 亮点，已删。） */
    ...
    for (int s = 0; s < k->ns; s++) {
        ...
        for (int v = 0; v < SEGS; v++) {
            ...
            /* ★★ 第 64 轮 v34：顶点扰动从"双频 cos + bite²"改成"三层噪声"。
               王总原话 + 参考图：v33 仍然是**椭圆形**（顶点围着椭圆中心走），
               真实色块是**多个分叶从中心伸出** + **锯齿边缘**。

               v_low = 0.20·cos(1.7θ+ph·1.3)         // 大形：仍是大致椭圆
               v_mid = 0.40·cos(4.3θ+hash·2π)       // **分叶**：4.3θ 一圈
                                                            约 4~5 个分叶，
                                                            hash 防周期
               v_hi  = 0.15·(2·hash−1)               // **锯齿**：每顶点独立
               bite  = v_low + v_mid + v_hi          // ∈[-0.75,+0.75]，均值≈0
               r     = 1 − bite                       // 大部分向内、偶有凸出

               ⇒ v_mid 和 v_hi 都是**非周期**的，角度和径向都打散 ⇒
                 顶点不再围绕椭圆中心 ⇒ 真正的不规则团块。

               ⚠️ 允许小幅凸出 r ∈ [0.25, 1.05] —— 完全只向内凹得不到尖角，
                  参考图的分叶就是凸出的尖。
               ⚠️ 凸出按**每个顶点自己的 lim_r** clamp，不能一刀切 1.05。
                  见下面 `lim_r` 计算 + if (r > lim_r) r = lim_r。 */
            /* ★ 第一版 v_mid 三层都双向，bite 标准差 0.47 ⇒ 太多顶点向内极深处跑
               ⇒ 单斑面积掉 74%。修法：
                 · v_mid 改为**只向内**（分叶本来就是向内凹的尖）：cos→0.5-cos/2，
                   保证 v_mid ∈ [0, 0.30]，均值 0.15 ⇒ 整体 bite 均值 0.15 ⇒
                   r 均值 0.85（接近 v33 的 0.83，单斑面积回 25~28px²）。
                 · v_hi 缩到 0.05（仅做高频微锯齿，不再贡献大凸出）；
...
/* ==========================================================================
   ...
   ■ ★ 第 45 轮那 14 点**不是凸的**（量过：叉积符号集 = {+1, -1}），
     当时靠"取 14 条边各自的内向半平面求交"绕开（交集天然凸，且 ⊆ 原多边形）。
     ★★ 第 58 轮换成的 8 点是**凸的**（叉积符号集 = {+1}），
       ⇒ 逐边半平面推回直接收敛，不再需要半平面交那套。
     ⚠️ 若哪天又改回非凸，必须重新验证"半平面交 ⊆ 多边形"与"内缩后非空"，
        别想当然。也别为"用回凸算法"去取凸包 —— 凸包比原多边形**大**。

   ■ 为什么用"逐边投影迭代"而不是"算最近边界点再推"
     交集是凸的 ⇒ 逐边投影（P ⟵ P − d·n，d<0 时）几轮就收敛，只用乘加，无 sqrt；
     而"最近边界点"要先遍历 SWIM_N 条边算距离再开方，还得分内外，贵且难写对。
