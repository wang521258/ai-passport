...
enum {
    ...
    PI_KGOLD_DK,     /* 阴影   #E0A63F = 224,166,63 —— 腹部 / 尾根压暗（59 轮提亮 +49） */
    PI_KGOLD_LT,     /* 亮部   #FFDE93 = 255,222,147 —— 头部略亮 */
    PI_KGOLD_HI,     /* 高光   #FFF3C2 = 255,243,194 —— 背部中央那条**很窄**的带 */
    PI_KSCALE,       /* 鳞片点 #FFF9DC = 255,249,220 —— 背上几粒很小的点 */
    PI_KGOLD_TL,     /* 尾梢浅金 #FFEAB0 = 255,234,176 —— 尾鳍"金黄→半透明浅金"的**那一端** */
...
static const uint8_t DAY_PAL[PI_NPAL][3] = {
    ...
    {244, 241, 232}, {229,  57,  45}, {248, 198,  94}, {252, 204, 176},
    ...
    /* ★★ 第 53 轮：御黄金的金属金层次（王总指定色值）
       ★★★ 第 59 轮：王总看真机后说「金色的鱼颜色还是有点深，需要浅一点，
            色差不要太大」⇒ **整体提亮 + 收窄跨度**，色相仍留在金色区间。

         档位        53 轮旧值            59 轮新值            亮度变化
         PI_KGOLD_DK #B96812 185,104,18 → #E0A63F 224,166,63   102 → 151  (+49)
         PI_KGOLD    #F5A623 245,166,35 → #F8C65E 248,198,94   149 → 180  (+31)
         PI_KGOLD_LT #FFD34E 255,211,78 → #FFDE93 255,222,147  181 → 208  (+27)
         PI_KGOLD_TL #FFE28C 255,226,140→ #FFEAB0 255,234,176  207 → 222  (+15)
         PI_KGOLD_HI #FFF0A0 255,240,160→ #FFF3C2 255,243,194  218 → 231  (+13)
         PI_KSCALE   #FFF7D1 255,247,209→ #FFF9DC 255,249,220  237 → 241  (+4)

       ■ 两个量化目标（_tools/goldcmp59.py 出对比图）
         · 明暗跨度  102→237（**135**） 收窄到 151→241（**90**）  −33%
         · 最大相邻差 47（DK→GOLD）     压到     29               −38%
         ⇨ "色差不要太大"这一条就是靠这两项。

       ■ 为什么提亮主要压在**暗端**（DK +49，HI 只 +13）
         "深"的观感几乎全来自最暗那一档 —— 它占鱼身面积最大（腹/尾根/鳍根）。
         高光端本来就接近白，再提会洗掉金属感。

       ■ 仍保留"金属金"读感的依据
         R−B（橙度）分别是 161/154/108/79/61/35，仍是从"橙金"渐变到"浅金"，
         不是一片死黄；RGB565 量化的 R 步长 8、G 步长 4，
         最小相邻差 29 在 R 上还有 3~4 级 ⇒ 层次不会被量化吃掉。
       ⚠️ 只给 pat==1（御黄金）用；红白那套（PI_KBODY/KSPOT）一个字节都不动。 */
    {224, 166,  63}, {255, 222, 147}, {255, 243, 194}, {255, 249, 220},
    {255, 234, 176},
