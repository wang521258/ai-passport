...
static void make_spots(koi_t *k)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        float ua = (n == 3) ? 0.80f : 1.70f;   /* 头斑更靠前（吻端方向） */
        float ub = 3.50f;
        float ut = (n > 1) ? ((float)i / (float)(n - 1)) : 0.0f;
        float u = ua + ut * (ub - ua) + (head - 0.30f) * 0.35f;
        if (u < 0.75f) u = 0.75f;
        if (u > 3.85f) u = 3.85f;
        int sg = (int)u;
        ...
        /* ★ v38b：n=3（头+中+后）时每块 **×0.82** —— 王总"斑是分开的"。
           实测斑半长 ≈0.73 段，而 n=3 相邻中心距只有 ≈1.0 段 ⇒ 重叠 0.46 段。
           缩小后半长 ≈0.60 段 ⇒ 重叠 0.2 段，再配合"左右交替在脊线两侧"
           ⇒ 看得出是分开的几块（真实锦鲤斑多的个体，每块本来就小一点）。 */
        if (n == 3) {
            sl *= 0.82f;
            sw *= 0.82f;
        }
