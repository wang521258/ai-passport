...
/* ==========================================================================
   ...
   ========================================================================== */
/* ★ PI_BAND 已删 —— 王总定"波带不要了"。水面现在只剩「纵向渐变 + 中心光晕 + 波光点」。
   删它的收益（真机读数）：CPU 307ms → 减少 112ms（条 75 + 波 37），脏区同时掉掉
   整屏宽的横条 —— 那个横条是脏区下不来的根因。 */
enum {
    PI_WTOP, PI_WBOT, PI_GLOW, PI_SPARK,
...
static const uint8_t NIGHT_PAL[PI_NPAL][3] = {
    ...
    {190, 198, 212}, {156,  78,  40}, {198, 164,  88}, {166, 132, 114},
    { 84,  50,  32}, {190, 222, 255}, {212, 180, 120},
...
/* ★ 水面波带（bands_draw / bands_mark_dirty）已整段删除 —— 王总定"波带不要了"。
   ...
   水面现在只剩「纵向渐变 + 中心光晕 + 44 个波光点」，全在 water_rect 里，一次画完。
...
/* ★ 荷叶画法的粗细档（28 轮）。王总原话：
   ...
   LILY_LV = 0  旧精细版（逐式照抄网页版）—— 留着只为对照，不要再当默认
   LILY_LV = 1  ★平面版（默认）：水下根影 + 叶身 + 1px 描边 + 缺口补点 + 叶芯
   ★ 平面版同时把荷叶的原语数从约 19 次 fill_poly/片 降到约 4 次/片 ——
     这正是真机那段 108ms 的大头（叶脉 11 条每条都是一次独立的 fill_poly）。 */
#define LILY_LV    1
...
static void lily_paint(const lily_t *L, float th)
{
    ...
    float r = L->r;
#if LILY_LV == 0
    /* 只有精细版用得上 —— 平面版不画叶脉/叶缘反光，声明一起包进来，
       否则 -Wunused-variable 会在 -Werror 下直接编不过。 */
    ...
    /* ★ 注意 s_lry 取的是 L->y，**不含 lily_bob** —— 也就是说"荷叶呼吸"其实没画出来：
       叶身固定在 L->y，只有伴生浮萍跟着 bob 上下（weed_paint 收到的 dy）。
       这一条是 27 轮顺着"荷叶凭什么每帧重画"查出来的：
       标脏一直按 L->y + bob 报，而画的是 L->y，等于每帧都在为一份不存在的位移报脏。
       本轮**故意不改** —— 给 s_lry 加上 lily_bob 会让荷叶真的开始呼吸，那是改画面，
       要单独一轮做、并且先出图给王总看。 */
    s_lrx = L->x; s_lry = L->y;
    ...
    /* ③ 叶脉 11 条：跳过缺口那一扇区；只到 0.94r，天然落在叶身内，不必裁剪。
       ⚠️ 这一段在固件上是**反面教材**：网页版用它画柔细的浅纹理（0.7px + 0.60），
       固件没有抗锯齿，`stroke_line` 最小 1px → 变成一条条实心暗线。
       保留只为对照，不要再当默认。 */
    for (int i = 0; i < 11; i++) {
...
#else
    /* ★ 平面版（王总 28 轮）：叶脉 / 老叶斑 / 叶缘反光**一律不画**。
       荷叶就是浮在水面上的一片扁平叶子 —— 从上方看它本来就没有立体细节，
       而且这 14 个多边形（11 条脉 + 2 块斑 + 1 道弧）正是真机 108ms 里的大头。
       保留：根影（上面①，唯一有效的"贴在水面"线索）+ 叶身 + 描边 + 缺口补点 + 叶芯。 */
#endif
