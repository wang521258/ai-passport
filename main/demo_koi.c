...
/* ★ 王总给的 14 点，原文照抄、一个数没动。顺序 = 上边左→右、右缘自上而下、
   ...
   ★★ 第 57 轮：长边外扩 10px，四个短边方向保持不变（整段平移、长度不变）：
     顶 y=20 → 10，底 y=298 → 308，左右外缘紧贴石头**未动**（dl=dr=0）。
     角点（pt 0/1/7/8）由平移后短边端点决定；
     短边端点也跟着平移（vector 锁死），所以 chamfer 仍平行于原方向。
     实测 polygon 面积 +3755 px²（+7.4%）；
     ★★ 用 marshmallow 在台架 + 真机分两步复核半平面交 ⊆ polygon / 内缩可容性。 */
static const float SWIM_PT[SWIM_N][2] = {
    { 42.0f,  10.0f}, {198.0f,  10.0f}, {211.0f,  38.0f}, {216.0f,  90.0f},
    {213.0f, 145.0f}, {218.0f, 205.0f}, {210.0f, 273.0f}, {192.0f, 308.0f},
    { 48.0f, 308.0f}, { 29.0f, 277.0f}, { 23.0f, 220.0f}, { 27.0f, 170.0f},
    { 23.0f, 115.0f}, { 29.0f,  52.0f},
};
