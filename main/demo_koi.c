...
/* ---- ★ 临时诊断：分段计时（量完就摘） --------------------------------------
   ...
   esp_timer_get_time() 每次约 1~3us，每帧 10 个点 ≈ 0.03ms，相对 307ms 可忽略。
   槽位：0=调色板 1=step 2=波带条循环 3=水 4=波带 5=鱼 6=涟漪 7=荷叶浮萍 8=饲料
   （3~8 是 scene_draw 内部，会被调用 s_nrect 次并累加；2 只测 render 的第一个循环） */
...
static void tick_cb(lv_timer_t *t)
{
    ...
    if (++s_log_acc >= FPS * 2) {
        ...
        /* ★ 临时诊断：每帧各段耗时(ms)。pal 调色板 / step 物理 / 条 波带条循环 /
           水 波带 鱼 涟 荷 料 = scene_draw 内部（已按调用次数累加，总和即 render 主体）。
           若 荷 那一项等于整段开销 → 是荷叶浮萍的绘制原语个数问题，不是填充问题。 */
        ESP_LOGI(TAG, "  剖 pal=%.2f step=%.2f 条=%.2f | 水=%.2f 波=%.2f 鱼=%.2f 涟=%.2f 荷=%.2f 料=%.2f",
                 (double)s_prof[0] / 1000.0 / n, (double)s_prof[1] / 1000.0 / n,
                 (double)s_prof[2] / 1000.0 / n, (double)s_prof[3] / 1000.0 / n,
                 (double)s_prof[4] / 1000.0 / n, (double)s_prof[5] / 1000.0 / n,
                 (double)s_prof[6] / 1000.0 / n, (double)s_prof[7] / 1000.0 / n,
                 (double)s_prof[8] / 1000.0 / n);
